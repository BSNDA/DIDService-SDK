...
public class Secp256Util {
  ...
  public static String sign(CryptoType cryptoType,String message, String privateKey) throws IOException {
	  if(cryptoType == CryptoType.SM) {
		  Sign.SignatureData signatureData = SM2Sign.sign(message.getBytes(StandardCharsets.UTF_8), GenCredential.createGuomiKeyPair(new BigInteger(privateKey).toString(16)));
		  return secp256k1SigBase64Serialization(signatureData);
	  }else {
		  ECDSASign ecdsaSign = new ECDSASign();
		  Sign.SignatureData signatureData = ecdsaSign.signMessage(message.getBytes(StandardCharsets.UTF_8), GenCredential.createECDSAKeyPair(new BigInteger(privateKey).toString(16)));
		  return secp256k1SigBase64Serialization(signatureData);
	  }
  }
   ...
   */
  public static boolean verify(CryptoType cryptoType, String message, String publicKey, String signValue) throws SignatureException {
