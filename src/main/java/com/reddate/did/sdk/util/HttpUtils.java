...
public class HttpUtils<T> {
	 ...
	 * 
	 */
	public static <T, E> ResultData<T> postCall(String url, String token,RequestParam<E> requestParam, Class<T> returnType) throws Exception {
...
//		client.dispatcher().setMaxRequestsPerHost(100);
		...
		if(httpResonseCode != HttpResponseStatus.OK.code()) {
			response.close();
			throw new RuntimeException("server process failed");
		}
		
		
		String resposneDataStr = response.body().string();
		response.close();
	...
	/**
	 * 
	 * call the OKHttp Get method to send some request data to server, and process server return data to the target type
	 * 
	 * 
	 * 
	 */
	public static <T> T getCall(String url, Class<T> returnType) throws Exception {
		Request request = new Request.Builder()
			      .url(url)
			      .get()
			      .build();
		...
		T returnData = JSONObject.parseObject(resposneDataStr,returnType);
		return returnData;
	}
